...
/*   Updated: 2022/07/21 14:11:37 by cpopa         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int	initialize_mutexes(t_data *data)
{
	...
	while (i < data->nr_philo)
	{
		...
		if (pthread_mutex_init(&data->philos[i].status_lock, NULL) != 0)
			return (error_mutex(data, "init status_lock fail\n"));
		if (pthread_mutex_init(&data->philos[i].meal_lock, NULL) != 0)
			return (error_mutex(data, "init meal_lock fail\n"));
		if (pthread_mutex_init(&data->philos[i].local_dead_lock, NULL) != 0)
			return (error_mutex(data, "init local_dead_lock fail\n"));
		i++;
	}
